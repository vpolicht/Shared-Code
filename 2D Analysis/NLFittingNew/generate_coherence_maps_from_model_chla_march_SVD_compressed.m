...
figures_path = fullfile(base,'2D','Frank','LyX_Lab_Notebook','LaserLab','14-03-11','FiguresCompressed');
data_file_name_r = 'data_stack_r_compressed.mat';
data_file_name_nr = 'data_stack_nr_compressed.mat';
...
mean_data_r = squeeze(mean(all_data_mat_r,3));
...
rmapsfull = zeros(size(all_data_mat_r,1),size(all_data_mat_r,2),best_model.Nosc);
% nrmapsfull = zeros(size(all_data_mat_r));
for k=1:best_model.Nosc;
...
% Ncontours = 50;
% ylim_low = dsearchn(y_axis_w_roi',wn2w(14705));
% xlim_low = dsearchn(x_axis_w_roi',wn2w(14000));
% ylim_high = dsearchn(y_axis_w_roi',wn2w(17857));
% xlim_high = dsearchn(x_axis_w_roi',wn2w(17000));
...
%     contourf(w2wn(x_axis_w_roi)*1e-3,w2wn(y_axis_w_roi)*1e-3,timg,v,'linewidth',0.01);
%     a = makeColorMapWStops(linspace(1,Ncontours,6),[[1,1,1]; [1,1,1]; [0,0,1]; [0,1,0]; [1,1,0]; [1,0,0]]);
%     colormap(a)
% %     colorbar;
%     hold all;
%     contour(w2wn(x_axis_w_roi)*1e-3,w2wn(y_axis_w_roi)*1e-3,abs(mean_data_r),linspace(min_bkg,0.5*max_bkg,8),'linewidth',0.5,'linecolor','k');
%     line(w2wn(x_axis_w_roi)*1e-3,w2wn(line_diag_eq(x_axis_w_roi,0))*1e-3,'linestyle','--','linewidth',0.5,'Color','k');
%     line(w2wn(x_axis_w_roi)*1e-3,w2wn(line_diag_eq(x_axis_w_roi,-wn2w(peaks(j))))*1e-3,'linestyle','--','linewidth',0.5,'Color','k');
%     line(w2wn(x_axis_w_roi)*1e-3,w2wn(line_diag_eq(x_axis_w_roi,+wn2w(peaks(j))))*1e-3,'linestyle','--','linewidth',0.5,'Color','k');
%     line(w2wn(x_axis_w_roi)*1e-3,w2wn(line_diag_eq(x_axis_w_roi,-2*wn2w(peaks(j))))*1e-3,'linestyle','--','linewidth',0.5,'Color','k');
%     xlim([14.0 17.0]);
%     ylim([14.705 17.857]);
...
% %     colorbar;
%     hold all;
%     contour(w2wn(x_axis_w_roi)*1e-3,w2wn(y_axis_w_roi)*1e-3,abs(mean_data_nr),linspace(min_bkg,0.5*max_bkg,8),'linewidth',0.5,'linecolor','k');
%     line(w2wn(x_axis_w_roi)*1e-3,w2wn(line_diag_eq(x_axis_w_roi,0))*1e-3,'linestyle','--','linewidth',0.5,'Color','k');
%     line(w2wn(x_axis_w_roi)*1e-3,w2wn(line_diag_eq(x_axis_w_roi,-wn2w(peaks(j))))*1e-3,'linestyle','--','linewidth',0.5,'Color','k');
%     line(w2wn(x_axis_w_roi)*1e-3,w2wn(line_diag_eq(x_axis_w_roi,+wn2w(peaks(j))))*1e-3,'linestyle','--','linewidth',0.5,'Color','k');
%     line(w2wn(x_axis_w_roi)*1e-3,w2wn(line_diag_eq(x_axis_w_roi,-2*wn2w(peaks(j))))*1e-3,'linestyle','--','linewidth',0.5,'Color','k');
