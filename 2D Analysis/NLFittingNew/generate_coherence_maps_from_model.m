...
figures_path = fullfile(base,'2D','Frank','LyX_Lab_Notebook','LaserLab','13-09-05','FiguresNew');
...
maps = zeros(dY,dX,best_model.Nosc);
for k=1:best_model.Nosc;
    maps(:,:,k) = By*reshape(cAmps(:,k),[Nbf Nbf])*Bx';
end
...
for j=1:best_model.Nosc;
    ...
    svgname = fullfile(figures_path,['beats_at_' num2str(peaks(j),3) '_wn.svg']);
    pdfname = fullfile(figures_path,['beats_at_' num2str(peaks(j),3) '_wn.pdf']);
