...
if ~exist('F','var');
    load(fullfile(data_path,'reduced_bkg_stack_no_oscillations.mat'));
end
if ~exist('rDm','var')
...
saveas(gcf,fullfile(figures_path,'frobenius_spectrum.png'));
dlmwrite(fullfile(figures_path,'frobenius_spectrum.txt'),[rZfaxis' frobeniusSpectrum],'\t');
...
%     saveas(gcf,fullfile(figures_path,'Beat_Analysis_Fourier',['beats_at_' num2str(peaks(k)) '_wn.eps']));
    plot2svg(fullfile(figures_path,'Beat_Analysis_Fourier',['beats_at_' num2str(peaks(k)) '_wn.svg']),fig);
%     setenv('PATH','C:\Program Files (x86)\Inkscape');
    export_string = ['inkscape ' fullfile(figures_path,'Beat_Analysis_Fourier',['beats_at_' num2str(peaks(k)) '_wn.svg'])  ...
        ' --export-png=' fullfile(figures_path,'Beat_Analysis_Fourier',['beats_at_' num2str(peaks(k)) '_wn.png']) ...
        ' --export-dpi=600'];
...
% Create textarrow
[fx1, fy1] = axescoord2figurecoord([145,148.06],[144, 147.18]);
