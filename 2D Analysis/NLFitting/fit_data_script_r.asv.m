...
fprintf(1,'%f\n',alpha(1:2:(2*Nosc-1)));
 ...
 save(fullfile(data_path,'reduced_bkg_stack_r_no_oscillations.mat'),'F','Bx','By','T');
...
if ~exist(fullfile(figures_path,'Beat_Analysis_Fourier_Rephasing'),'dir');
    system(['mkdir' ' ' fullfile(figures_path,'Beat_Analysis_Fourier_Rephasing')]);
end
if ~exist('kristin_excitons','var');
    load(fullfile(data_path,'kristin_excitons'));
end
if ~exist('renger_excitons','var');
    load(fullfile(data_path,'renger_excitons'));
end
if ~exist('novo_excitons','var');
    load(fullfile(data_path,'novo_excitons'));
end
peaks = [91.24 127.1 338.9 250.9 729.9 853.7 974.3];
% peaks = [338.9];
...
for k=1:length(peaks);
    figure('units','inches','position',[3 3 3 3]);
...
%         line(w2wn(x_axis_w_roi)*1e-2,ones(size(x_axis_w_roi))*D_excitons(r)*1e-2,'linestyle','-.','linewidth',1.5,'Color','r');
%         line(w2wn(x_axis_w_roi)*1e-2,ones(size(x_axis_w_roi))*D_excitons(c)*1e-2,'linestyle','-.','linewidth',1.5,'Color','r');
%         line(ones(size(y_axis_w_roi))*D_excitons(r)*1e-2,w2wn(y_axis_w_roi)*1e-2,'linestyle','-.','linewidth',1.5,'Color','r');
%         line(ones(size(y_axis_w_roi))*D_excitons(c)*1e-2,w2wn(y_axis_w_roi)*1e-2,'linestyle','-.','linewidth',1.5,'Color','r');
%     end
    if peaks(k)<max(max(renger_excitons - diag(diag(renger_excitons))));
        line(w2wn(x_axis_w_roi)*1e-2,ones(size(x_axis_w_roi))*D_excitons_r(r1)*1e-2,'linestyle','-','linewidth',1.5,'Color','r');
        line(w2wn(x_axis_w_roi)*1e-2,ones(size(x_axis_w_roi))*D_excitons_r(c1)*1e-2,'linestyle','-','linewidth',1.5,'Color','r');
        line(ones(size(y_axis_w_roi))*D_excitons_r(r1)*1e-2,w2wn(y_axis_w_roi)*1e-2,'linestyle','-','linewidth',1.5,'Color','r');
        line(ones(size(y_axis_w_roi))*D_excitons_r(c1)*1e-2,w2wn(y_axis_w_roi)*1e-2,'linestyle','-','linewidth',1.5,'Color','r');
    end
    if peaks(k)<max(max(novo_excitons - diag(diag(novo_excitons))));
        line(w2wn(x_axis_w_roi)*1e-2,ones(size(x_axis_w_roi))*D_excitons_n(r2)*1e-2,'linestyle','--','linewidth',1.5,'Color','r');
        line(w2wn(x_axis_w_roi)*1e-2,ones(size(x_axis_w_roi))*D_excitons_n(c2)*1e-2,'linestyle','--','linewidth',1.5,'Color','r');
        line(ones(size(y_axis_w_roi))*D_excitons_n(r2)*1e-2,w2wn(y_axis_w_roi)*1e-2,'linestyle','--','linewidth',1.5,'Color','r');
        line(ones(size(y_axis_w_roi))*D_excitons_n(c2)*1e-2,w2wn(y_axis_w_roi)*1e-2,'linestyle','--','linewidth',1.5,'Color','r');
    end
    ...
    saveas(gcf,fullfile(figures_path,'Beat_Analysis_Fourier_Rephasing',['beats_at_' num2str(peaks(k)) '_wn.eps']));
    hold off;
end
