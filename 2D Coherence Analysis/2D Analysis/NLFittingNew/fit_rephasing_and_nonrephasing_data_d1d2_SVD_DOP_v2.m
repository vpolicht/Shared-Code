...
fprintf(1,'Rephasing Data is loaded... \n')
...
fprintf(1,'NonRephasing Data is loaded... \n')
...
OscBounds.sub = 1.0;
OscBounds.slb = 0.1;
RateBounds.rub = [1/50 1/500];
RateBounds.rlb = [1/1500 1/3000];
RateBounds.sub = 1.0;
RateBounds.slb = 0.1;
MaxNOsc = 12;
MinNOsc = 8;
MaxNdynRates = 2;
MinNdynRates = 1;
MaxConstRates = [1/3500 1/8000];
%All the possible models
[ModelCell, Diagnostics] = GenManySoLStretchedModels(MaxNOsc, MinNOsc, MaxNdynRates, MinNdynRates, MaxConstRates, OscBounds, RateBounds, T);
...
parfor k=1:length(ModelCell)
    ...
    beta_start(1:3:(Nosc*3)) = 1./T2wn(candidate_freqs(randi([1,length(candidate_freqs)],[1,Nosc]))); %#ok<PFBNS>
    beta_start(3:3:(Nosc*3)) = 1;
...
save(fullfile(results_path,['best_model_' s '.mat']),'best_model','-v7.3');
