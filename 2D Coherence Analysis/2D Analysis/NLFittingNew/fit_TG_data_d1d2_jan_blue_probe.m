...
% data_path = fullfile(base,'2D','Data','14-02-02','d1d2_tg-batch09','processed_signal');
...
if phase_it
    ...
    TG = TGSignal;
    W = vWe;
    [Theta, Alpha, Beta] = MinimizationPhasing(TG, PP, W, [1 1340], [0 -200 0]', [(2*pi-eps) 200 500]', 10);
...
if exist('Theta','var')
    Corr = (Alpha*repmat(exp(1i*Theta),[1 length(Trange)]))';
    Y = real(Y.*Corr);
else
    Y = abs(Y);
end
...
OscBounds.rub = rub;
OscBounds.rlb = rlb;
RateBounds.rub = [1/40  1/200 1/1500  1/12000];
RateBounds.rlb = [1/300 1/2000 1/15000 1/50000];
% RateBounds.rub = [1/50  1/200  1/600  1/2000];
% RateBounds.rlb = [1/300 1/1000 1/3000 1/6000];
MaxNOsc = 0;
MinNOsc = 0;
MaxNdynRates = 6;
MinNdynRates = 1;
