...
%
% /Volumes/Utini/Data/TMD ML/WS2/5-21-2020/WS2-spot1-1000fs-pu1uJ-pr880nJ-1_DToverT.dat     SM_GRA10('CALLBACK',hObject,eventData,handles,...) calls the local
...
% Last Modified by GUIDE v2.5 06-Mar-2019 11:06:14
...
if strcmp(st,'insert T2 value') || ~T2value
    ...
    if (get(handles.checkboxNM,'Value') == get(handles.checkboxNM,'Max'))
        wlpump = (299792./(SlopeCal.*XX+InterceptCal));
    else
       wlpump = converter./(299792./(SlopeCal.*XX+InterceptCal));
    end
...
%         for ii = 1:floor(length(wprCut)/2^ProbeAverageNsquare)
%             wprCutaver(ii) = mean(wprCut(ii*2^ProbeAverageNsquare-2^ProbeAverageNsquare+1:ii*2^ProbeAverageNsquare));
%             DataCutaver(:,ii) = mean(DataCut(:,ii*2^ProbeAverageNsquare-2^ProbeAverageNsquare+1:ii*2^ProbeAverageNsquare),2);
%         end

        wprCutaver = wprCut;
        DataCutaver = smoothdata(DataCut,2,'movmean',2*ProbeAverageNsquare);
...
%             UniGaussWindow = UniGaussWindow(length(x)/2+1:end);
        ...
        for ii = 1:size(Data_in,2)
            SpikeInd = find(abs(Data_in(:,ii))>spike_thrsh);
            Data_in(SpikeInd,ii) = 0;
            Data_inFilter(:,ii) = Data_in(:,ii).*Filter; %%multiply each column for the filter
        end
        ...
        % Pump Filter
        DataCut2D = Map2D(IndexPump,:); %Data cut according to pump wavelength
        ...
        if (get(handles.checkboxNBP,'Value') == get(handles.checkboxNBP,'Max'))
            for ii = 1:size(DataCut2D,2)  
                DataCut2D(:,ii) = DataCut2D(:,ii)./(Pump_spectrum);
                ps  =  1;
            end
        end
    ...
    save([pat(1:tdt(end)-1) 'complete-' pst pat(tdt(end):end)],'wlpump','B','Map2DSave','T2In','wprCutaver')
...
if get(handles.invax,'Value') == get(handles.invax,'Max') % invert axis
    if get(handles.ucm,'Value') == get(handles.ucm,'Max') % 1/cm
        B = flipud(B);
        B = fliplr(B);

        contourf(wprCutaver,wlpump,permute(B,[2 1]))
        colormap parula
        shading flat
        hold on
        [C,hfigc] = contour(wlpump, wprCutaver, BwprCutaver,wlpump,permute(B,[2 1]),20);
        set(hfigc,'LineWidth',1.0,'Color', [1 1 1]);
        xlabel('\nu_{3} [cm-1]');
        ylabel('\nu_{1} [cm-1]'); 
        daspect([1 1 1])
    else
        contourf(fliplr(wlpump),fliplr(wprCutaver),permute(B,[2 1]),20)
%         colormap parula
%         shading flat
%         hold on
%         [C,hfigc] = contour(fliplr(wprCutaver),fliplr(wlpump),permute(B,[2 1]),20);
%         set(hfigc,'LineWidth',1.0,'Color', [1 1 1]);
    ...
    if get(handles.ucm,'Value') == get(handles.ucm,'Max') % 1/cm
        B = flipud(B);
        B = fliplr(B);
        pcolor(wprCutaver,wlpump,permute(B,[2 1]))
        colormap parula
        shading flat
        hold on
        [C,hfigc] = contour(wprCutaver,wlpump,permute(B,[2 1]),20);
        set(hfigc,'LineWidth',1.0,'Color', [1 1 1]);
        xlabel('\nu_{1} [cm-1]');
        ylabel('\nu_{3} [cm-1]'); 
        daspect([1 1 1])
    else
        contourf((wlpump),(wprCutaver),permute(B,[2 1]),20)
%         pcolor(wlpump,wprCutaver,permute(B,[2 1]))
%         colormap parula
%         shading flat
%         hold on
%         [C,hfigc] = contour(wlpump,wprCutaver,permute(B,[2 1]),20);
%         set(hfigc,'LineWidth',1.0,'Color', [1 1 1]);
...
hold on 
line(wlpump,wlpump,'Color','k','LineStyle','--','LineWidth',1);
hold off
...
function pushbutton10_Callback(hObject, eventdata, handles)
 ...
 if (get(handles.radiobutton3,'Value') == get(handles.radiobutton3,'Max'))
    ...
    for ii = 1:length(T2In)
        A = cell2mat(Map2DSave(ii));
        B(ii) = A(IndexYcursor,IndexXcursor);
    end
...
if (get(handles.radiobutton4,'Value') == get(handles.radiobutton4,'Max'))
    ...
    for ii = 1:length(wprCutaver)
        if vettpump1(ii)>dim(1) || vettpump1(ii)<=0
            dvett(ii) = 0;
        else
            dvett(ii) = A(vettpump1(ii),ii);
        end

        if vettpump2(ii)>dim(1) || vettpump2(ii)<=0
            avett(ii) = NaN;
        else
            avett(ii) = A(vettpump2(ii),ii);
        end
    end
    ...
    for J = 1:N
        ...
        for ii = 1:length(wprCutaver)
            if vettpump1(ii)>dim(1) || vettpump1(ii)<=0
                ...
                Tdiag(ii,J) = A(vettpump1(ii),ii);
            end

            if vettpump2(ii)>dim(1) || vettpump2(ii)<=0
                Toff(ii,J) = 0;
            else
                Toff(ii,J) = A(vettpump2(ii),ii);
            end
        end
    end
...
% --- Executes on button press in but_OPA3.
function but_OPA3_Callback(hObject, eventdata, handles)
% hObject    handle to but_OPA3 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of but_OPA3